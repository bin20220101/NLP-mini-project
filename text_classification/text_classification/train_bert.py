...
def main_task(args, train_args):
    ...
    # 加载 bert-base-uncased 模型, 并设置模型输出的类别数
    model = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=len(news_loader.target_names))
    # 初始化 Trainer
    trainer = Trainer(
        model,
        train_args,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='bert', help='模型保存路径')
    ...
    train_args = TrainingArguments(
        ...
        logging_steps=100,  # 每 100 steps 时, 记录 log
        save_steps=100,  # 每 100 steps 时, 保存模型
        learning_rate=2e-5,  # 学习率
        per_device_train_batch_size=16,  # 训练时的 batch_size
        per_device_eval_batch_size=16,  # 验证时的 batch_size
        num_train_epochs=5,  # epoch, 训练的轮数
